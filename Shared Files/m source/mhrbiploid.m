function [WPI,MPI,PPI] = mhrbiploid(obj,col4row,y,P_D,params)
...
%        mentioned as the original author of the code.  
    ...
    WPI = zeros(1,J);
    MPI = zeros(params.xn_dim,J);
    PPI = zeros(params.xn_dim,params.xn_dim,J);
    
    % compute IPL-OID approximation for each GMM component
    for j = 1:J
        % obtain gamma threshold from a lookup table
        gammaT = gamma_table(sum(col4row(:,j) <= m_k)+1);
        mpi = msp;
        Ppi = Psp;
