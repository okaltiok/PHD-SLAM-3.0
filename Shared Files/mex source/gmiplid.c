...
void update(double *ms, double *Ps, double *A, double *b,  double *Omega,  double *y);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    const mwSize ndim = 3;
    const mwSize dims3D[3] = {xn_dim,xn_dim,J+1};
    
    plhs[0] = mxCreateDoubleMatrix(1, J+1, mxREAL);
    plhs[1] = mxCreateDoubleMatrix(xn_dim, J+1, mxREAL);
            ...
            *gamma = 0.0;
            ...
            for (int i=0; i < n_k; i++)
            {
                if ((int)col4row[i] <= m_k)
                {
                    ...
                    update(ms, Ps, A, b, Omega,  Y + (int)((col4row[i]-1)*h_dim));
                }   
                xl += xl_dim;
                Pl += xl_dim*xl_dim;
            }
            ...
            if(!choleskyFactorization(invPs, Ps, xn_dim))
            {
                done = 1;
            }
            else
    ...
    // compute weight of prior and append to GMM
    double weight_threshold[1] = {0.0};
    for (int i=0; i < n_k; i++)
        *weight_threshold += log(1 - exp(P_D[i])) + eta[i];
    
    // copy prior to output
    memcpy(WPI, weight_threshold, sizeof(double));
    memcpy(MPI, msp, XN_DIM_SOD);
    memcpy(PPI, Psp, XN_DIM2_SOD);
}
