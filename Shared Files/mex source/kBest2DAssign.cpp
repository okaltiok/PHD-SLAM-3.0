 ...
 *          is false.
 * threshold If given, the assignment problem is terminated once the ratio
 *           between the current assignment and the first assignment is
 *           below a threshold, gainBest(1) - gainBest(curSweep) < threshold.
 *           The default if this parameter is omitted is -Inf       
 *
...
/*
 * The original code has been modified to terminate hypothesis generation 
 * whenever the likelihood of the next hypothesis is below some fixed  
 * percentage of the likelihood of the first (most likely) hypothesis as 
 * described in [4].
 *
 * [4] I. Cox and M. Miller, “On finding ranked assignments with application
 * to multitarget tracking and motion correspondence,” IEEE Transactions
 * on Aerospace and Electronic Systems, vol. 31, no. 1, pp. 486–489, 1995.
 *
 * Author of the modification : Ossi Kaltiokallio
...
void mexFunction(const int nlhs, mxArray *plhs[], const int nrhs, const mxArray *prhs[]) {
    ...
    if(nrhs==4) {
        threshold=getDoubleFromMatlab(prhs[3]);
    }
     ...
     * solutions exist.*/
    numFound=kBest2D(k,numRow,numCol,maximize, mxGetDoubles(CMat), workMem, col4rowBest,row4colBest,gainBest,threshold);
    mxDestroyArray(CMat);
