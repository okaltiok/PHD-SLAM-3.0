...
% computing the cost matrix between the vectors in x_mat and y_mat
% has beed changed from the original code to reduce computational overhead
a = x_mat;
b = y_mat;
aa=sum(a.*a); bb=sum(b.*b); ab=a'*b; 
cost_mat = sqrt(repmat(aa',[1 size(bb,2)]) + repmat(bb,[size(aa,2) 1]) - 2*ab);
cost_mat(cost_mat > c) = c;


% below is the original code to compute the cost matrix 
